#!/usr/bin/env python
# -*- encoding: utf-8 -*-
'''
@Python     :   3.9
@Date       :   2024/4/2 15:13


with chat_placeholder.container  用法见https://docs.streamlit.io/develop/api-reference/layout/st.empty
能持续刷新页面。如果把L85行替换成 with container() message..，会出现message 一直更新闪烁的情况，暂时未解决。这里先用原生的st.write_stream， 在下一次输入会渲染成message

'''
import streamlit as st
from streamlit_chat import message
from zhipuai import ZhipuAI

ZHIPU_API_KEY = "填入你的zhipuai的token"

GLM_LOGO = "data:image/png;base64,"
PEOPLE_LOGO = "https://chatglm.cn/img/user_head.49ce25cd.svg"

if "past" not in st.session_state:
    st.session_state.past = []

if "generated" not in st.session_state:
    st.session_state.generated = []

if "role_desc_input" not in st.session_state:
    st.session_state.role_desc_input = ""

st.set_page_config(
    page_title="GLM-4演示版本",
    layout="wide"
)
st.title("GLM-4演示版本")
st.text_input("请输入角色相关的描述（可为空）", key="role_desc_input")



def on_input_change():
    user_input = st.session_state.user_input
    if user_input:
        st.session_state.past.append(user_input)
        ask_glm()

def on_clear_msg_click():
    del st.session_state.past[:]
    del st.session_state.generated[:]

ZHIPU_AI_CLIENT = ZhipuAI(api_key=ZHIPU_API_KEY)  # 请填写您自己的APIKey


def ask_glm():
    history = [
        {"role": "system", "content": st.session_state.role_desc_input}] if st.session_state.role_desc_input else []
    for i in range(len(st.session_state['past'])):
        history.append({"role": "user", "content": st.session_state['past'][i]})
        if i == len(st.session_state['past']) - 1:
            continue
        history.append({"role": "assistant", "content": st.session_state['generated'][i]})
    response = ZHIPU_AI_CLIENT.chat.completions.create(
        model="glm-4",  # 填写需要调用的模型名称
        messages=history,
        stream=True,
        temperature=0.99,
        # sensitive_word_check={"type": "ALL", "status": "DISABLE"}
    )
    st.session_state.generated.append(response)

def parse_glm_2_str(data):
    for chunk in data:
        yield chunk.choices[0].delta.content

chat_placeholder = st.empty()
with chat_placeholder.container():
    for i in range(len(st.session_state['generated'])):
        message(st.session_state['past'][i], is_user=True, key=f"{i}_user", logo=PEOPLE_LOGO)
        if isinstance(st.session_state['generated'][i], str):
            message(
                st.session_state['generated'][i],
                key=f"{i}_glm", logo=GLM_LOGO,
                allow_html=True,
                is_table=False
            )
        else:
            r = st.write_stream(parse_glm_2_str(st.session_state['generated'][i]))
            st.session_state['generated'][i] = "".join(r)

with st.container():
    st.button("清空消息", on_click=on_clear_msg_click)
    st.text_input("用户输入:", key="user_input", on_change=on_input_change)
